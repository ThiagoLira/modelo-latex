...
\begin{equation}
   Var(y^* | x^*) = Var[\mathop{\mathbb{E}}(y^* | W,x^*)] + \mathop{\mathbb{E}}[Var(y^* | W,x^*)]
   \label{eq:vartot}
 \end{equation}


$\mathbb{E}(y^* | W,x^*)$ é a verossimilhança do modelo, ou seja:

\[
  Var[\mathbb{E}(y^* | W,x^*)] = Var[f^W(x^*)]
\]


A estimativa de $Var[f^W(x^*)]$ será feita pela técnica do MC Dropout. A variância
...
\[   Var[f^W(x^*)] \sim \eta_1^2 = \frac{1}{B}\sum^B_{B=1}(\hat{y}^*_{(B)} - \hat{y}^*)^2  \]

O termo $\mathop{\mathbb{E}}[Var(y^* | W,x^*)]$ da Equação~\ref{eq:vartot}, o
...
\[
  \mathop{\mathbb{E}}[Var(y^* | W,x^*)] \sim \eta_2^2 = \frac{1}{V}\sum^V_{V=1}(y'_v - f^W(x'_v))^2
\]
...
A verossimilhança do modelo, $p(y_{i,t} | \mu,\sigma)$, é uma distribuição fixa cujos parâmetros são dados por uma função $\omega$ da saída $h$ do modelo. Usaremos uma função de verossimilhança binomial já que estamos otimizando um objetivo de regressão, assim como na Equação~\ref{eq:logver1}:
\[
...
O modelo possui uma etapa de treinamento e uma etapa de validação. O Diagrama~\ref{deepartrain} ilustra como o modelo se comporta em cada um desses momentos: \\


\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\columnwidth]{deepar-treino.png}
...
\[
  \mathcal{L} = argmax \sum_i{\log p(y_i | \mu,\sigma)}
\]
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\columnwidth]{deepar-pred.png}
