...
E de fato, o objetivo de treino da maior parte dos modelos de Aprendizado Automático podem ser escritos dessa forma caso eles não possuam regularização~\cite{dlbook}.

O \textbf{custo} de um modelo é medido por esse valor agregado de todo os dados de treino. A otimização para Aprendizado Supervisionado é feita usualmente pelo algoritmo de Backpropagation \citep{dlbook}, usando o gradiante dessa função para buscar um mínimo local.

\[
  \mathbb{L} = \prod_{i=0}\log p( y_i | x_i ; \theta)
\]
