...
recalcular os parâmetros das regressões e repetindo-se o processo com $T' = T + t_f$.
...
$[T-29,T-29 + t_f]$, visto que usamos os modelos por $t_f$ dias, então novas
anotações de RC28 se tornaram disponíveis. 


\[
  \mathit{diff}[i] = reglin\_7[i] - RC28[i]
\]

É aplicada então uma média móvel exponencial no vetor diff, com parâmetro
$\alpha$, cujo resultado é o vetor filtrado diff\_ew.


Seja um vetor $\textbf{Y}$, calcula-se a sua média móvel exponencial
$\textbf{Y}^*$ pelo algoritmo definido na Equação~\ref{eq:ew}.
...
reglin\_1 e $i \in [T-29,T-29+t_f]$ para diff\_ew. O parâmetro $k$ também se
torna um hiperparâmetro a ser escolhido.

\[
reglin\_ew[t] = reglin\_1[t] + k * \mathit{diff\_ew}[i] 
\]
