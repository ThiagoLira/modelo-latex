...
entrada contém $W$ entradas do vetor das variáveis. O parâmetro $W$, o
...
\[
\mathcal{L} = \prod_{t=W}^{i=T} p(y_t | \textbf{x}_{(t-W):t}, \theta) 
\]
...
\[
  \mathcal{L} = \sum_{t=W}^{i=T} \log{p(y_t | \textbf{x}_{(t-W):t},\theta)}
\]
...
\[
  \mathcal{L} = \prod_{t=W}^{i=T}p(y_t|\textbf{x}_{(t-W):t},\theta)p(\theta) 
\]
