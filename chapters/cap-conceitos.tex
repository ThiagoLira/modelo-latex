...
A seguir segue uma breve definição do problema que esse trabalho busca
solucionar comparando diversos métodos. Os termos usados nesse capítulo serão
...
independentes $x_i$ com valores nos instantes de tempo no intervalo $[1,T]$. Seja $T$ o último dia em que se possui dados de ambas essas
...
$\mathcal{L}$ é dado por \citep{dlbook}:
...
hipótese arbitráriamente próxima da verdade.
...
A Função de Custo é a função usada para guiar os algoritmos de otimização na
direção de parâmetros que melhorem a performance do modelo \citep{dlbook}. Nessa
otimização, como explicado na Sessão~\ref{sec:reglog}, estamos aumentando a
verossimilhança apresentada pelo modelo escolhido. 
\\
