...
de probabilidade do tipo $p(y | x)$, ou seja, para diversos exemplos de vetores
$x$ são fornecidas anotações $y$, e desejamos então criar predições de anotacoes
$y'$ para novas entradas $x'$. Muitos algoritmos resolvem esse problema por meio
da busca pelo conjunto de parâmetros $\theta$ que minimizem o erro na família de
distribuições $p(y | x;\theta)$, i.e. parametrizadas por $\theta$.
...
\[MSE = \sum^n_{i=1}\frac{(x_i - y_i)^2}{n}\]
