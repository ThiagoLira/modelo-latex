...
\section{O problema de predição de RC como Aprendizado Supervisionado}
...
\subsection{Regressão de Séries Temporais}
...
\subsection{Estimadores Frequentistas}
\label{sec:est}


Um estimador dos parametros $\theta$, na visão frequentista, é uma função dos dados, e portanto uma
variável aleatória. Considerando-se os dados como uma realização de uma
distribuição fixa porém desconhecida, o objetivo é encontrarmos um estimador
que, no limite, se aproxime dos parâmetros corretos \cite{mlprob}.
Esse parâmetro pode ser a média da distribuição do alvo, por exemplo.

Embora qualquer função dos dados possa ser usada como um estimador, bons
estimadores podem ser encontrados pelo método da Máxima Verossimilhança (MLE).
Isso significa, computar a probabilidade de um certo estimador
...
verossimilhança, e essa mudança torna a otimização mais estável.

\[
  \theta_{MLE} = \argmax_{\theta}\log p( Y | X ; \theta)
\]

O objetivo também pode ser escrito por um produtório da verossimilhança de todos
os pares de dados, assumindo que todos são independentes:

\[
  \theta_{MLE} = \argmax_{\theta}\prod_{i=0}\log p( y_i | x_i ; \theta)
\]

E de fato, o objetivo de treino da maior parte dos modelos de Aprendizado
Automático podem ser escritos dessa forma, caso esses não possuam regularização~\cite{dlbook}. 
...
Essa função para alguns modelos pode representar o cálculo da verossimilhança do
modelo, mas isso não é necessário no geral.

\subsubsection{Erro Quadrático Médio}
\label{sec:MSE}

O Erro Quadrático Médio é um estimador não enviesado da verossimilhança de um
modelo Gaussiano \cite{dlbook}. Ele é calculado em relação a duas variáveis contínuas
...
Daremos um exemplo de um desses métodos, o Monte Carlo Dropout, que não requer nenhuma mudança na arquitetura do modelo para realizar a inferência. Consistindo apenas de uma reinterpretação de uma Rede Neural implementada com uma técnica de regularização chamada Dropout. 
...
otimização da rede com dropout é equivalente ao objetivo da Inferência Variacional.
...
(i.e. parâmetros da distribuição de probababilidade):
...
distribuição dos parâmetros da rede \citep{dropbayes}. O Processo Gaussiano
Profundo é um modelo flexível que permite escrever em termos probabilisticos uma
grande gama de modelos de Aprendizado Profundo (CITACAO).
Seja $\mathnormal{L}$ o
