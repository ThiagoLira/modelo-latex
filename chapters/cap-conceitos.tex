...
Uma rede LSTM possui 4 conjuntos de parâmetros internamente. O sinal
$c_t$ está conectado diretamente na entrada e na saída da rede, e tem seu valor
dependente de $i_t$ e $f_t$. Por meio do treinamento a rede pode aprender que
tipo de informação é útil para ser propaga pela via $c_t$, aprendendo a
\say{lembrar} e \say{esquecer} de acordo com o problema em questão.
\\

As equações que ditam o funcionamento da LSTM na iteração temporal $t$ são: \\
...
É nessa equação que a rede atualiza a sua célula de memória, apagando ou
propagando alguma informação para a proxima iteração temporal. \\
...
\caption{ Diagrama de Rede Encoder-Decoder.\\ Retirado de \cite{encdec}}
...
entradas, essa última forma chamada de \textit{teacher forcing}. 
