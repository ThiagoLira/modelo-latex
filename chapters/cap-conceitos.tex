...
tratá-los como uma série temporal, i.e. usaremos o tempo como uma informação
...
Supervisionado(Tópico explicado na Sessão~\ref{sec:aprenauto}): Buscamos a distribuição das anotações $y$ condicionada
aos parâmetros de entrada $x$, mas também ao
histórico da série temporal, i.e. valores passados do objetivo $y$ e dos valores
passados do vetor de entrada $\textbf{x}$. 
...
observações das diversas variáveis  $x_i$ e do alvo $y$ no passado
...
regressão linear pode ser entendida como a distribuição $p(y | x) = \theta^Tx + \epsilon =
...
uma janela móvel de dados, i.e. para a predição do índice RC28 de um dia $t$ é
realizada uma regressão pelo método dos mínimos quadrados para o conjunto de
dados desde o dia $t -29 -t_d$ até $t -t_d$, $t_d$ sendo um parâmetro
selecionado após testes.
Esse intervalo é escolhido visto que um novo resultado de RC28 de um lote de
cimento demora 29 dias após a expedição desse lote,
então poderíamos usar no dia $t$ apenas a informação do RC28 de um lote
28 dias no passado.

Esse modelo treinado na faixa temporal $[t -29 -t_d,t - t_d]$ é usado para predições em um
horizonte de tempo definido por um outro parâmetro escolhido pelo programador,
$t_f$. Usamos o modelo na faixa temporal $[t -29 -t_d,t -t_d]$ para prever o índice RC28
do intervalo $[t,t+t_f-1]$. O modelo é retreinado com uma nova janela sempre que um novo valor de
$t$ ultrapassa $t + t_f -1$ do último modelo treinado.
