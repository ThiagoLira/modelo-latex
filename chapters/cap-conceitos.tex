...
\label{sec:exp}
Algoritmos de Aprendizado Automático podem ser divididos nas categorias de aprendizado supervisionado, não supervisionado e aprendizado por reforço \citep{dlbook}. Técnicas dos dois primeiros tipos serão usadas para os dados da Intercement.
...
mas isso não é necessário para muitos modelos. 
...
De acordo com o Teorema de Gauss-Markov, encontramos o modelo dessa forma com a
menor variância possível pelo método dos mínimos quadrados \citep{bayesml}.  
O modelo assume que existe uma relação entre a entrada e a saída é linear, somando-se um termo
de erro normal com média zero e variância constante.
...
É importante notar que a Equação~\ref{eq:pq} é verdade para qualquer escolha de
$q$. Em \citep{dropbayes} escolhe-se uma distribuição altamente multi-modal.
...
Pode-se notar que outra distância KL está presente no objetivo de otimização, a
distância entre a distribuição a priori $p(\theta)$ e a distribuição aproximada $q(\theta)$ dos parâmetros do modelo.
...
Seja uma rede neural com apenas uma camada, cuja matriz de parâmetros é $\theta$, 
como explicado na Sessão~\ref{sec:nn}. Amostrar uma máscara para o vetor de
ativação a cada computação da rede é equivalente a considerar que estamos
amostrando os próprios pesos da rede a cada computação de uma
distribuição $q(\theta)$ \citep{dropbayes}. Definimos a distribuição $q(\theta)$
da seguinte maneira:

\newcommand{\diag}{\mathop{\mathrm{diag}}}

\[
  \theta_i   = M_i   diag (z_{i,j}) 
\] 
\[
...
No trabalho \citep{dropbayes}, é demonstrado que a aplicação do dropout em todas as camadas da rede faz com que a minimização
do objetivo de treinamento seja equivalente a maximização do valor
$\mathnormal{L_{VI}}$ na Equação~\ref{eq:ine}, i.e. o objetivo da Inferência Variacional.\\

O termo $\sum^N_{N=1} (y_n - \mathnormal{f}^{\theta^*,b}(x_n))^2$ é equivalente
a uma aproximação por Monte Carlo da integral no primeiro termo da equação,
considerando apenas um termo, i.e. $\theta^* \sim q(\theta)$ 
...
A verossimilhança do modelo, $p(y_{i,t} | \omega(h_{i,t}))$, é uma distribuição fixa cujos parâmetros são dados por uma função $\omega$ da saída $h$ do modelo. Usaremos uma função de verossimilhança binomial já que estamos otimizando um objetivo de regressão, assim como na Equação~\ref{eq:logver1}:
