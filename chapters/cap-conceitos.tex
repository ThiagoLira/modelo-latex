...
Um modelo de aprendizado supervisionado $f$ com parâmetros $\theta$ poderá ter
duas formas, uma auto-regressiva e uma sem auto-regressão, i.e. a própria série
alvo usada no problema de predição. 
...
E para a versão sem auto-regressão:

\[
  \hat{RC28}_{T:T+F}  = f ( \textbf{x}_{T-W:T+F} ; \theta)
\]
...
o último valor de RC28 que pode ser usado está presente 29 dias no passado, característica
incorporada nos modelos auto-regressivos, atrasando a série alvo em 29 dias.
...
calcular uma aproximação da distribuição das predições $q(y^*|x^*)$. A
aproximação da equação a seguir depende de uma boa escolha de $q$. 
...
mas não o seu erro de treinamento}. A ideia é de penalizar o modelo pela sua
...
Após $t_f$ dias um novo dataset é escolhido para recalcular os parâmetros das
regressões e repete-se o processo com $T' = T + t_f$. A Figura~\ref{explic1}
ilustra, para um dia $T$, quais dados serão usados para treino e validação do
modelo.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\columnwidth]{regexplic1.png}
  \caption{Diagrama representando o uso do modelo de regressão linear janelado
    na prática para os dados.}
  \label{img:explic1}
\end{figure}
...
\subsection{Consumo de dados de Cimento para Modelos baseados em RNNs}


\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\columnwidth]{dlmodel.png}
  \caption{Diagrama com uma exemplificação do uso de modelos sequenciais
    baseados em RNNs para a predição de resistência RC28.}
  \label{fig:rnngraph}
\end{figure}
