...
rede Forecaster após o treino. Como estamos lidando com regressão, assume-se: \\
 ...
 Bayesiana para marginalizar os parâmetros e calcular a distribuição $p(y^* | x^*)$, e também
 explicamos como aproximar esse cálculo usando a técnica do MC Dropout. A
 variância dessa distribuição quantifica a incerteza das predições
...
na rede com camadas de Dropout ativadas. Seja $\{y^*_{(1)},y^*_{(2)}, \dots
y^*_{(B)}\}$ o vetor de predições amostrado dessa forma, e $\hat{yˆ*}$ sua
média amostral, temos: \\

\[   Var(f^W(x^*))  = \frac{1}{B}\sum^B_{B=1}(y^*_{(B)} - \hat{yˆ*})^2  \]
