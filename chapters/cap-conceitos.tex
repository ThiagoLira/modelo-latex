...
Queremos realizar uma predição de uma série temporal. Ou seja, dado valores passados queremos prever o comportamento da série num horizonte de tempo futuro finito. Iremos alterar a formalização do tópico~\ref{sec:apren} condicionando então a distribuição a valores passados do objetivo $y$. A série temporal é composta por diversas medições indexadas pelo tempo $t$ do objetivo de predição $y$ e de um vetor $\textbf{x}$ de parâmetros. Os dados de treinamento serão os pares $(\{\textbf{x}_{t_0},y_{t_o}\},\{\textbf{x}_{t_1},y_{t_1}\}, \dots, \{\textbf{x}_{T},y_{T}\})$ no intervalo de tempo $[t_o,T]$. Seja um horizonte finito de tempo $F$, $F > T$, os modelos devem aprender uma distribuição de probabilidade $p(y_{T:F} | y_{t_{o}:T},\textbf{x}_{t_{0}:T})$.
...
Esse processo se chama Inferência Variacional e seu objetivo (aproximar a distribuição variacional pela distribuição real $p(\theta | X,Y)$)
é aproximado minimizando-se a seguinte inequação, o \textit{Variational Lower Bound}: \\
...
\cite{dropbayes} demonstram que a aplicação do dropout em todas as camadas da rede faz com que a minimização do objetivo de treinamento também
represente a minimização da Inequação~\ref{eq:ine}, i.e. o objetivo da Inferência Variacional.\\
...
dessa medida é calculada então pela variância amostral de $Y^*$. E sua média pela média amostral.
