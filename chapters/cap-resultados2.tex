...
Usamos a implementação dos modelos DeepAR e DeepFactors da bibliteca GluonTS
\citep{gluonts}. De modo análogo ao modelo de Regressão Linear Dinâmica, para
cada modelo de DL iremos treinar 3 versões distintas para cada conjunto de variáveis apresentado na Tabela~\ref{tab:modelslin}.


Para o modelo DeepAR, as versões treinadas serão chamadas de \textbf{DeepAR\_1},
\textbf{DeepAR\_3} e \textbf{DeepAR\_7}. Os outros modelos usados seguirão uma
regra de nomeação análoga.

As Figuras~\ref{fig:fordeepar1},\ref{fig:fordeepar3} e \ref{fig:fordeepar7} reportam as
predições geradas em cada dataset de treino por cada versão do modelo DeepAR.
...
Sessão~\ref{sec:quant}. Os custos quantílicos dos modelos são reportados nas Tabelas~\ref{tb:quants1} e \ref{tb:quants2}.
...
\end{tabular}
  \caption{Riscos-quantílicos para os percentis de 50\% e 90\% das distribuições
    das predições para o modelo DeepAR.}
  \label{tb:quants1}
\end{table}
\end{center}
...
\begin{table}
  \begin{tabular}{l|l|l|l|l|l|l|}
    \cline{2-7}
    & \multicolumn{3}{l|}{P50QL}                       & \multicolumn{3}{l|}{P90QL}                       \\ \cline{2-7} 
    & DeepFactors\_7 & DeepFactors\_3 & DeepFactors\_1 & DeepFactors\_7 & DeepFactors\_3 & DeepFactors\_1 \\ \hline
    \multicolumn{1}{|l|}{7d}  & 0.06           & 0.13           & 0.05           & 0.03           & 0.053          & 0.04           \\ \hline
    \multicolumn{1}{|l|}{24h} & 0.06           & 0.13           & 0.05           & 0.03           & 0.053          & 0.04           \\ \hline
  \end{tabular}
  \caption{Riscos-quantílicos para os percentis de 50\% e 90\% das distribuições
    das predições para o modelo DeepFactors.}
  \label{tb:quants2}
\end{table}
...
comparar o erro quadrático das predições. Iremos comparar esses resultados com os obtidos pelo modelo
