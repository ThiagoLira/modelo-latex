...
Much of the past work cite:cementlin,nncement using mathematical models to predict CSC have failed to account for the dynamical nature of the industrial process and instead rely on analysis that are hard to make 
...
We shall bring these new Bayesian Deep Learning techniques to the CSC prediction domain. The contributions of this paper are 
as follows: 
