...
Our data doesn't conform to some core assumptions of classical time series models, such as stationarity and homoscedasticity, 
making models such as ARIMA cite:inbook not good choices for fitting the data. Another limitation of ARIMA models is their inability to model non-linear relationships 
on the data cite:forecasting.

Many time series problems on different domains had it's state of the art results beaten by new Deep Learning Recurrent Neural Network models cite:energylstm,lstmbr,
these deep learning models require no /feature engineering/ and work very well even with gigabytes of data cite:ARIMA_LSTM. They are more data-driven than classical methods, meaning that little manual 
tweaking on the model parameters is necessary.

Classic time series models also fail to scale well to huge amounts of data, although they are able to provide good uncertainty estimates cite:deepar. Past work cite:DIAZROBLES20088331,KHASHEI2010479 
...
#+BEGIN_EXPORT latex
\begin{equation*}
\sum^n_{t}\sqrt{\frac{(y_t - y^*_t)^2}{n}}
\end{equation*}
#+END_EXPORT
...
The Encoder-Decoder models seems to be able to best model the target distribution, although all models (after some weeks) settle on a stable mean prediction of the process. 
This graph could be used to model trends regarding the industrial process on a short horizon of time.

To evaluate the quality of the uncertainty measures, we shall use the .5 risk and .9 risk metrics. For each model 
we wil compare the risks for the predictions of the next day, the next 3 days and the next 7 days. (The bigger the number the worse is the uncertainty).
...
All this work accounts for the reality of the industrial process, and could be implemented in any cement factory with similar daily measurements.
