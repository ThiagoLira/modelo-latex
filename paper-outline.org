...
All models were implemented using PyTorch cite:pytorch, for the Gaussian Proccesses we used GPyTorch cite:gpytorch. The hyperparameters chosen for each model are shown on Table [?]. 
