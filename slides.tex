...
\item Custos de manutenção da ordem de bilhões de dólares dão importância
  estratégica para previsão da resistência de cimento, possivelmente ainda
  durante sua produção.
\item Avanços recentes no campo do Deep Learning criaram ferramentas que
...
\begin{frame}
  ...
  \begin{empheq}[box=\tcbhighmath]{align*}
   ...
   \widetilde{\mathit{Var}}[y^*]  &= \frac{1}{B}\sum^B_{B=1}(\hat{y}^*_{(B)} - \hat{y}^*)^2 
  \end{empheq}
