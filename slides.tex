...
\begin{empheq}[box=\tcbhighmath]{align*}
 L_{VI} = \int q(W) \log p(Y | X,W)dW - \mathnormal{KL}(q(W) || p(W))
\end{empheq}
...
\begin{frame}
  ...
  \begin{empheq}[box=\tcbhighmath]{align*}
    L_{dropout} = \sum^N_{N=1} (y_n - \mathnormal{f}^{\hat{W}_1,\hat{W}_2,b}(x_n))^2 + \boldsymbol{\alpha}\{W_1^2,W_2^2,b^2\}
  \end{empheq}
...
&\approx \frac{1}{T}\sum^T_{t=1}\log p(y^*| x^*, \hat{W}_t )
\end{empheq}
...
\begin{frame}
  ...
  \begin{empheq}[box=\tcbhighmath]{align*}
   \widetilde{\mathbb{E}}[y^*|x^*] &= \frac{1}{B}\sum^B_{B=1}\hat{y}^*_{(B)} \\ 
   \widetilde{\mathit{Var}}[y^*| x^*]  &= \frac{1}{B}\sum^B_{B=1}(\hat{y}^*_{(B)} - \bar{y}^*)^2 
  \end{empheq}
